...
@export var damage_per_level: PackedFloat64Array = [2.0, 3.0, 4.0, 8.0]
@export var count_per_level: PackedInt32Array = [1, 2, 3, 6]
...
var bonus_proj_speed: float = 0
const BONUS_EVOLVED_PROJ_SPEED: float = 1.0
...
func _spawn_dagger(dmg: float) -> void:
	...
	dagger.damage = dmg
	dagger.speed_mult = (stats.get_projectile_speed() / 300.0) + bonus_proj_speed # Normalize based on base speed
...
func _evolve() -> void:
	bonus_proj_speed += BONUS_EVOLVED_PROJ_SPEED
